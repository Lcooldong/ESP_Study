...
#define MESSAGE "Hello GCamp\n"
#define TCPServerIP  "52.215.34.155"  //"echo.mbedcloudtesting.com" to IP
...
void tcp_client(void *pvParam){
	...
	//    tcpServerAddr.sin_port = htons( 5500 );
	...
	while(1){
		...
		do {
			...
			for(int i = 0; i < r; i++) {
				putchar(recv_buf[count++]);
			}
		} while( count < strlen(MESSAGE) );
		ESP_LOGI(TAG, "... socket read success");
...
void app_main()
{
	...
	//If the previously set WIFI SSID and PASSWORD information is stored, 
	//the network is connected before entering the setting value with BluFI, 
	//so initialize the flash memory.
	//Comment after BlueFI Testing
