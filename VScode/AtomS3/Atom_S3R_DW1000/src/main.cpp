...
#define RX2_PIN GPIO_NUM_1
#define TX2_PIN GPIO_NUM_2
    ...
    switch (UWB_MODE) {
        case 0:  // Tag mode 标签模式
            if (UWB_T_NUMBER > 0 && UWB_T_NUMBER < 5) {
                ...
                while (c > 0) {
                    c--;
                    M5.Display.drawString("B", 20, 50 + c * 40, 4);               // Base station 基站
                    M5.Display.drawString(DATA.substring(2 + c * 11, 3 + c * 11), 37, 50 + c * 40, 4);  // Tag the serial number 标签序号
                    M5.Display.drawString(DATA.substring(4 + c * 11, 8 + c * 11), 210, 50 + c * 40, 4);  // Distance 距离
                }
                while (b > 0) {
                    b--;
                    M5.Display.fillRect(0, 170 - b * 40, 340, 30, BLACK);
                }
            }
            break;
        case 1:  // Base station mode 基站模式
            if (UWB_B_NUMBER == 1) {
                M5.Display.drawString(DATA, 60, 90,
...
void UWB_ui_display()
{
    M5.Display.drawString("UWB Example", 90, 0, 4);  // UI
    M5.Display.drawString("Tag", 50, 210, 4);
    M5.Display.drawString("Base", 130, 210, 4);
    M5.Display.drawString("Reset", 220, 210, 4);
    ...
    switch (UWB_MODE) {
        case 0:
            if (UWB_T_NUMBER > 0 && UWB_T_NUMBER < 5) {  // Tag mode UI display  标签模式UI显示
                ...
                while (c > 0) {
                    c--;
                    M5.Display.drawString("B", 20, 50 + c * 40, 4);
                    M5.Display.drawString("Distance:", 80, 50 + c * 40, 4);
                    M5.Display.drawString("M", 280, 50 + c * 40, 4);
                }
        ...
        case 1:  // Base station mode UI display  基站模式UI显示
            M5.Display.drawString("Base station", 80, 50, 4);
            M5.Display.drawString(String(UWB_B_NUMBER), 240, 50,
                              4);  // UWB_B_NUMBER
            if (UWB_B_NUMBER == 0) {
                M5.Display.drawString("loading......", 100, 90, 4);
            } else {
                M5.Display.fillRect(0, 90, 340, 30, BLACK);
            }
...
void UWB_clear()
{
    ...
    timer_flag = 0;
    timer_data = 0;
    M5.Display.fillRect(0, 50, 340, 150, BLACK);
}
    ...
    switch (UWB_MODE) {
        case 0:
            ...
            if (timer_data == 0 || timer_data > 8) {  // Count the number of Base stations
                                                      // 提示与基站0断连（测试）
                if (timer_data == 9) {
                    M5.Display.fillRect(210, 50, 50, 30, BLACK);
                }
                DATA       = "  0 2F   ";
                timer_flag = 0;
            }
...
void setup() {
  ...
  delay(3000);
  Serial.begin(115200);
  Serial2.begin(115200, SERIAL_8N1, RX2_PIN, TX2_PIN);  // UWB serial port 超宽频串口
  delay(100);
  UWB_Timer();
  UWB_ui_display();
...
void loop() {
  ...
  if (millis() - lastMillis >= 100) {
    lastMillis = millis();
    Serial.println("UWB_MODE: " + String(UWB_MODE) + " | DATA: " + DATA + " | UWB_T_NUMBER: " + String(UWB_T_NUMBER) + " | UWB_B_NUMBER: " + String(UWB_B_NUMBER) + " | timer_data: " + String(timer_data));
  }
