...
{
  if(touchLastTime > RESET_DEADLINE)
  {
    ESP.restart();
  }
...
// OLED
  ...
  u8x8.clearDisplay();
  u8x8.setCursor(0, cursorPosition++);
  u8x8.setFont(u8x8_font_amstrad_cpc_extended_r);
  if (WiFi.SSID().length() > 16)
  {
    u8x8.print(WiFi.SSID().substring(0, 16));
  }
